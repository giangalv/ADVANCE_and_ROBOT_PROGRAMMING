...
int write_log(char *to_write, char type)
{
    ...
    // if the type is 'i', we write an information message
    if (type == 'b'){
        sprintf(log_buffer, "%d:%d:%d - COMMAND process botton pressed: %s\n", tm.tm_hour, tm.tm_min, tm.tm_sec, to_write);
    }
    // if the type is 'e', we write an error message
    else if (type == 'e'){
        sprintf(log_buffer, "%d:%d:%d - COMMAND process error: %s\n", tm.tm_hour, tm.tm_min, tm.tm_sec, to_write);
    }
